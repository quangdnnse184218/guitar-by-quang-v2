...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function setAccountEmail(email) {
    if (accountEmailSpan) {
      accountEmailSpan.textContent = email || 'Tài khoản hợp lệ'
    }
  }
  ...
  function setLoading(isLoading) {
    ...
    if (isLoading) {
      ...
      if (btnText) btnText.textContent = 'Đang lưu mật khẩu...'
      if (btnSpinner) btnSpinner.classList.remove('hidden')
    } else {
      submitBtn.classList.remove('opacity-70', 'cursor-not-allowed')
      if (btnText) btnText.textContent = 'Lưu Mật Khẩu Mới'
  ...
  async function checkSession() {
    try {
      ...
      console.warn('[reset-password] Session check error:', err)
    }
  }
  ...
  if (form) {
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        showAlert('Đặt lại mật khẩu thành công! Đang chuyển hướng về trang đăng nhập...', true)
        ...
        console.error('[reset-password] Update password error:', err)
