...
export const DEFAULT_GEARS = [
  {
    ...
    description:
      'Mặt Sitka Spruce, lưng hông Rosewood. Tiếng mộc dày, âm bass ấm và action được căn rất êm tay.',
    price: 'Liên hệ',
    buyUrl: '',
    buy_url: '',
    link: '',
  ...
  },
  {
    ...
    description:
      'Mic thu cắm cổng USB trực tiếp vào máy tính, thu âm mộc qua Audacity, chỉ chỉnh âm lượng chứ không can thiệp hiệu ứng.',
    price: '2.500.000đ',
  ...
  },
  {
    ...
    description:
      'Dây phủ nanoweb bấm êm tay, lâu rỉ. Kẹp kèm Capo Shubb C1B bằng đồng chống phô nốt.',
    price: '420.000đ',
  ...
  },
  {
    ...
    description:
      'Phần mềm để mình viết tab, xuất file nhạc và căn chỉnh nhịp phách chi tiết trước khi quay video.',
    price: 'Bản quyền',
    buyUrl: '',
    buy_url: '',
    link: '',
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      if (isEdit && gearId) {
        ...
        const colMatch =
          error.message?.match(/column "([^"]+)" of relation "gears" does not exist/i) ||
        ...
        const { data, error } = await supabase.from('gears').insert([attemptPayload]).select()
...
export async function saveGear(payload, isEdit = false, gearId = null) {
  ...
  const { data: savedRecord, error: supabaseError } = await writeGearToSupabase(
    cleanPayload,
    isEdit,
    gearId
  )
  ...
  if (supabaseError) {
    console.error(
      '[gears-service] Không thể lưu vào Supabase:',
