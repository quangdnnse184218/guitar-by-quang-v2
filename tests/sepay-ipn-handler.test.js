...
function fakeClient() {
  return {
    from(table) {
      ...
      const rows = () => {
        const source =
          table === 'orders' ? db.orders : table === 'purchases' ? db.purchases : []
      ...
      const run = async () => {
        ...
        if (table === 'bank_transactions') {
          ...
          if (dup && ctx.op === 'upsert' && ctx.opts?.ignoreDuplicates) return { data: [], error: null }
...
describe('sepay-ipn — luồng khớp đơn (hành vi gốc)', () => {
  it('đơn đang chờ + đủ tiền → đánh dấu đã trả, cấp tab, ghi log matched', async () => {
    ...
    expect(db.bankTransactions).toEqual([
      expect.objectContaining({ external_id: '9001', status: 'matched', order_code: 'DH123456', amount: 239000 }),
    ])
  })
...
describe('sepay-ipn — tiền về mà không khớp phải được LƯU và BÁO', () => {
  it('thiếu tiền → không cấp tab, lưu unmatched/amount_low, gửi email cho admin', async () => {
    ...
    expect(db.purchases).toHaveLength(0)
    expect(db.bankTransactions[0]).toMatchObject({ status: 'unmatched', reason: 'amount_low', order_code: 'DH123456' })
    expect(emails).toHaveLength(1)
    expect(emails[0].subject).toContain('Tiền về chưa khớp đơn')
  })

  it('nội dung không có mã đơn → unmatched/no_code', async () => {
    const r = await deliver(bank({ content: 'chuyen tien an trua' }))
    expect(r.body.reason).toBe('no_code')
    expect(db.bankTransactions[0]).toMatchObject({ status: 'unmatched', reason: 'no_code', order_code: null })
  })
  ...
  it('SePay gửi lại đúng giao dịch đó → chỉ một dòng log và chỉ một email', async () => {
    await deliver(bank({ content: 'khong ma don' }))
    await deliver(bank({ content: 'khong ma don' }))
  ...
  it('email không bị chèn HTML từ nội dung chuyển khoản do khách tự gõ', async () => {
    await deliver(bank({ content: '<img src=x onerror=alert(1)>' }))
...
describe('sepay-ipn — an toàn', () => {
  it('tiền RA có chứa mã đơn hợp lệ KHÔNG được coi là thanh toán', async () => {
    ...
    expect(db.orders[0].status).toBe('pending')
    expect(db.purchases).toHaveLength(0)
    expect(db.bankTransactions[0]).toMatchObject({ status: 'ignored', reason: 'outgoing' })
